...
export default function ElaveSections({ data = [], onChange, userTier = 'Free', cvLanguage = 'azerbaijani' }: ElaveSectionsProps) {
  ...
  const addOrganizationsSection = () => {
    ...
    const organizationsSection: CustomSection = {
      id: Date.now().toString(),
      title: siteLanguage === 'english' ? 'Organizations' : 'Təşkilatlar',
      elements: [
        {
          id: `org-${Date.now()}-1`,
          type: 'heading',
          content: siteLanguage === 'english' ? 'Organization Name' : 'Təşkilat Adı'
        },
        {
          id: `org-${Date.now()}-2`,
          type: 'text',
          content: siteLanguage === 'english' ? 'Position/Role' : 'Vəzifə/Rol'
        },
        {
          id: `org-${Date.now()}-3`,
          type: 'dateRange',
          content: siteLanguage === 'english' ? 'Duration' : 'Müddət',
          subContent: siteLanguage === 'english' ? '2020 - Present' : '2020 - Hazırda'
        },
        {
          id: `org-${Date.now()}-4`,
          type: 'text',
          content: siteLanguage === 'english' 
          ...
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                📝 {getTranslation(siteLanguage, 'text')}
              </span>
              <button
            ...
            <textarea
              ...
              placeholder={getTranslation(siteLanguage, 'textPlaceholder')}
      ...
      case 'list':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                📋 {getTranslation(siteLanguage, 'list')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <div className="space-y-2">
              {element.items?.map((item, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <input
                    ...
                    placeholder={`${getTranslation(siteLanguage, 'listItemPlaceholder')} ${index + 1}`}
              ...
              >
                {getTranslation(siteLanguage, 'listItemAdd')}
              </button>
            </div>
          </div>
        );
      ...
      case 'heading':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                🏷️ {getTranslation(siteLanguage, 'heading')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <input
              ...
              value={element.content}
              onChange={(e) => updateElement(sectionId, element.id, { content: e.target.value })}
              placeholder={getTranslation(siteLanguage, 'headingPlaceholder')}
      ...
      case 'dateRange':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                📅 {getTranslation(siteLanguage, 'dateRange')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              <input
                ...
                value={element.content}
                onChange={(e) => updateElement(sectionId, element.id, { content: e.target.value })}
                placeholder={getTranslation(siteLanguage, 'startDatePlaceholder')}
              ...
              <input
                ...
                placeholder={getTranslation(siteLanguage, 'endDatePlaceholder')}
                className="p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              />
            </div>
          </div>
      ...
      case 'achievement':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                🏆 {getTranslation(siteLanguage, 'achievement')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <input
              ...
              value={element.content}
              onChange={(e) => updateElement(sectionId, element.id, { content: e.target.value })}
              placeholder={getTranslation(siteLanguage, 'achievementTitlePlaceholder')}
            ...
            <textarea
              value={element.subContent || ''}
              onChange={(e) => updateElement(sectionId, element.id, { subContent: e.target.value })}
              placeholder={getTranslation(siteLanguage, 'achievementDetailsPlaceholder')}
      ...
      case 'skill':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                ⭐ {getTranslation(siteLanguage, 'skill')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              <input
                ...
                value={element.content}
                onChange={(e) => updateElement(sectionId, element.id, { content: e.target.value })}
                placeholder={getTranslation(siteLanguage, 'skillNamePlaceholder')}
                className="p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              />
              <select
                value={element.subContent || getTranslation(siteLanguage, 'skillLevels').beginner}
                onChange={(e) => updateElement(sectionId, element.id, { subContent: e.target.value })}
                className="p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              >
                <option value={getTranslation(siteLanguage, 'skillLevels').beginner}>{getTranslation(siteLanguage, 'skillLevels').beginner}</option>
                <option value={getTranslation(siteLanguage, 'skillLevels').intermediate}>{getTranslation(siteLanguage, 'skillLevels').intermediate}</option>
                <option value={getTranslation(siteLanguage, 'skillLevels').advanced}>{getTranslation(siteLanguage, 'skillLevels').advanced}</option>
                <option value={getTranslation(siteLanguage, 'skillLevels').expert}>{getTranslation(siteLanguage, 'skillLevels').expert}</option>
              </select>
            </div>
          </div>
        );
      ...
      case 'link':
        return (
          <div key={element.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 flex items-center">
                🔗 {getTranslation(siteLanguage, 'link')}
              </span>
              <button
                onClick={() => removeElement(sectionId, element.id)}
                className="text-red-400 hover:text-red-600 transition-colors"
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              <input
                ...
                value={element.content}
                onChange={(e) => updateElement(sectionId, element.id, { content: e.target.value })}
                placeholder={getTranslation(siteLanguage, 'linkTitlePlaceholder')}
                className="p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              />
              <input
                type="url"
                value={element.subContent || ''}
                onChange={(e) => updateElement(sectionId, element.id, { subContent: e.target.value })}
                placeholder={getTranslation(siteLanguage, 'linkUrlPlaceholder')}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="min-w-0 flex-1">
          <h3 className="text-base sm:text-lg font-semibold text-gray-900">{getTranslation(siteLanguage, 'sectionTitle')}</h3>
          <p className="text-xs sm:text-sm text-gray-600 break-words">{getTranslation(siteLanguage, 'sectionDescription')}</p>
        </div>
        <div className="flex items-center space-x-2 flex-shrink-0">
          ...
          >
            ...
            <span className="hidden sm:inline text-sm">
              {getTranslation(siteLanguage, 'addSection')}
            </span>
            <span className="sm:hidden text-sm">
              {getTranslation(siteLanguage, 'addSectionMobile')}
            </span>
          </button>
        </div>
      </div>
      ...
      {sections.length === 0 ? (
        <div className="text-center py-8 sm:py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          ...
          <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">{getTranslation(siteLanguage, 'noSectionsTitle')}</h3>
          <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 px-4 break-words">{getTranslation(siteLanguage, 'noSectionsDescription')}</p>
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-3">
            ...
            >
              {getTranslation(siteLanguage, 'addFirstSection')}
            </button>
          </div>
        </div>
      ) : (
                ...
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-1 sm:space-x-2 flex-1 min-w-0 group">
                    <input
                      ...
                      placeholder={getTranslation(siteLanguage, 'newSectionPlaceholder')}
                    />
                    <svg 
                ...
                <div className="p-3 sm:p-4">
                  <div className="space-y-4">
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 sm:p-4">
                      <h4 className="text-xs sm:text-sm font-medium text-gray-700 mb-3">{getTranslation(siteLanguage, 'addElement')}</h4>
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                        {getElementTypes(cvLanguage, siteLanguage).map((type) => (
                          <button
                            key={type.type}
