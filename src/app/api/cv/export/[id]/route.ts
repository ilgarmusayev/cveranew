...
async function generatePDF(browser: any, cvData: any, templateId: string, fontSettings: any, htmlContent: string, cssContent: string, cvId: string) {
    ...
    try {
                ...
                <head>
                    ...
                    <style>
                        ...
                        * {
                        ...
                        @page {
                            size: A4;
                            margin: 8mm 6mm 23mm 6mm;  /* Top, Right, Bottom (8mm + 15mm reserve), Left */
                        }
        ...
        // SMART A4 PAGE BREAK ALGORITHM - 15mm bottom reserve
        await page.evaluate(() => {
            console.log('=== A4 SMART PAGE BREAK ALGORITHM STARTING ===');
            
            // A4 calculations with 15mm bottom reserve
            const a4HeightMM = 297; // A4 height in mm
            const topMarginMM = 8;
            const bottomMarginMM = 8;
            const bottomReserveMM = 15; // User requested 15mm bottom reserve
            const totalBottomSpaceMM = bottomMarginMM + bottomReserveMM; // 23mm total
            
            const a4UsableHeightMM = a4HeightMM - topMarginMM - totalBottomSpaceMM; // 297 - 8 - 23 = 266mm
            const mmToPx = 3.779527559; // 1mm = 3.779527559px at 96 DPI
            const a4UsableHeight = a4UsableHeightMM * mmToPx; // 266mm = ~1005px
            const pageBreakThreshold = Math.floor(a4UsableHeight * 0.95); // 95% threshold = ~950px
            
            console.log('A4 Page Calculations:');
            console.log('- A4 height:', a4HeightMM, 'mm');
            console.log('- Top margin:', topMarginMM, 'mm');
            console.log('- Bottom margin + reserve:', totalBottomSpaceMM, 'mm');
            console.log('- Usable height:', a4UsableHeightMM, 'mm =', Math.round(a4UsableHeight), 'px');
            console.log('- Page break threshold:', pageBreakThreshold, 'px');
            
            // Find template container
            const template = document.querySelector('.cv-preview, .cv-template, .template-container, [class*="template"]');
            
            if (template) {
                const templateHeight = template.scrollHeight;
                console.log('Template height:', templateHeight, 'px');
                ...
                if (templateHeight > a4UsableHeight) {
                    ...
                    (template as HTMLElement).style.height = 'auto';
                    (template as HTMLElement).style.minHeight = 'auto';
                    (template as HTMLElement).style.maxHeight = 'none';
                    (template as HTMLElement).style.overflow = 'visible';
                    (template as HTMLElement).style.pageBreakInside = 'auto';
                    (template as HTMLElement).style.breakInside = 'auto';
                    ...
                    allElements.forEach((element, index) => {
                        ...
                        if (elementTop > 0 && elementTop > (pageNumber * pageBreakThreshold)) {
                            ...
                            console.log(`Page break ${pageBreaksAdded}: Added before element at ${elementTop}px (Page ${pageNumber})`);
        ...
        const pdfBuffer = await page.pdf({
            ...
            margin: {
                ...
                bottom: '23mm',   // 8mm alt boşluq + 15mm reserve = 23mm
